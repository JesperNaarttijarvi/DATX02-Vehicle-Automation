...
class ParticleFilter:
    ...
    def step_time(self, id, measurement_vector, most_likely_states, interval, precalculation, grantlist):
        ...
        #Resample from measurement if all weights are 0 EME
        if w_sum == 0:
            print("*******************Weights sum to 0, resample from measurement")
            new_particles = self.generate_inital_particles(measurement_vector)
            new_weights = [1.0 / self.n_particles] * self.n_particles
        else:
