...
def get_lane_pos(lines, offset=170):
...
def draw_lane_lines(image, lines, color):
...
def image_publisher():
    ...
    while(cap.isOpened()):
        ...
        # ROS image stuff
        frame = np.uint8(combo_frame3)
        image_message = bridge.cv2_to_imgmsg(combo_frame3, encoding="passthrough")
